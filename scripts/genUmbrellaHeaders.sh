...
top="mu"
exclude_pat='*_EXCLUDE_*'
exclude_file_pat_def='*_def.hpp'
exclude_file_pat_decl='*_decl.hpp'
...
process_dir() {
  ...
  done < <(find "$dir" -mindepth 1 -maxdepth 1 -type d ! -name "$exclude_pat" -print0 | LC_ALL=C sort -z)
  ...
  while IFS= read -r -d '' f; do
    ...
    find "$dir" -mindepth 1 -maxdepth 1 -type f -name '*.hpp' \
      ! -name "$exclude_pat" \
      ! -name "$exclude_file_pat_def" \
      ! -name "$exclude_file_pat_decl" \
  ...
  if (( local_count < 1 && child_count == 0 )); then
