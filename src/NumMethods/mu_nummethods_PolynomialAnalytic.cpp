...
inline Vectord solveScalarQuadraticEq(double a, double b, double c) {
  if(a==0)
    return Vectord(vector<double>{-c/b, -c/b});
  double underSqrt = b*b - 4*a*c;
  if(underSqrt < 0) {
    MU_THROW("");
    return Vectord();
  }
  else if(underSqrt == 0) return Vectord(vector<double>{-b/(2*a), -b/(2*a)});
  //else if(underSqurt > 0)
    if(a>0) return Vectord(vector<double>{(-b + sqrt(underSqrt))/(2*a), (-b - sqrt(underSqrt))/(2*a)});
    if(a<0) return Vectord(vector<double>{(-b - sqrt(underSqrt))/(2*a), (-b + sqrt(underSqrt))/(2*a)});
